...
1. If I'm at `domain.com/signup` and I push `"/confirm"` to a route adapter, it should go to `domain.com/confirm`.
2. If I'm at `domain.com/signup` and I push `"confirm"` (no prefixed slash) to a route adapter, it should go to
   `domain.com/signup/confirm`.
3. If I'm at `domain.com/signup` and I push `"./confirm"` (prefixed with current directory) to a route adapter, it
   should go to `domain.com/signup/confirm`.
4. If I'm at `domain.com/signup` and I push `"../confirm"` (prefixed with move up one directory) to a route adapter, it
   should go to `domain.com/confirm`.
5. If I'm at `domain.com/signup/confirm` and I push `"../complete"` (prefixed with move up one directory) to a route
   adapter, it should go to `domain.com/signup/complete`.
6. If I'm at `domain.com/signup/complete` and I push `"../../login"` (prefixed with move up two directories) to a route
   adapter, it should go to `domain.com/login`.
