import { AuthInfo, CloudFunctionEventTransformer, NormalizedCloudFunctionEventData } from './Types';
...
export const getHeadersFromEvent = (event: IAWSCloudFunctionEvent): Record<string, string[]> => {
  const { headers = {}, multiValueHeaders = {} } = event;
  const mergedHeaders: Record<string, string | string[]> = {
    ...headers,
    ...multiValueHeaders,
  };
  const normalizedHeaders: Record<string, string[]> = Object.keys(mergedHeaders).reduce((acc, key) => {
...
export const getAuthInfo = (event: IAWSCloudFunctionEvent): AuthInfo => {
  const {
    requestContext: {
      authorizer: { claims: { sub: userId = undefined, 'cognito:groups': roles = [] } = {} } = {},
    } = {},
  } = event;
