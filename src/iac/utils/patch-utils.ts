      ...
      if (mergeStrategy === "accumulate") {
        ...
        for (let i = 0; i < existingValue.length; i++) {
          ...
          if (existingItem && typeof existingItem === "object") {
            const identifier =
              mergeStrategyData instanceof Function
                ? mergeStrategyData(existingItem)
                : existingItem[mergeStrategyData as string];
        ...
        for (let j = 0; j < newValue.length; j++) {
          ...
          if (newItem && typeof newItem === "object") {
            const identifier =
              mergeStrategyData instanceof Function
                ? mergeStrategyData(newItem)
                : newItem[mergeStrategyData as string];
